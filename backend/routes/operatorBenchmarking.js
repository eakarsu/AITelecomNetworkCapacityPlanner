...
// Operator-benchmarking dashboard comparing peer carriers
// Feature: operator-benchmarking
...
router.post('/analyze', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Operator-benchmarking dashboard comparing peer carriers.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'operator-benchmarking', payload, result);
    res.json({ success: true, feature: 'operator-benchmarking', ...result });
  } catch (err) {
    console.error('operator-benchmarking analyze error:', err.message);
...
router.post('/batch', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Operator-benchmarking dashboard comparing peer carriers. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'operator-benchmarking', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authMiddleware, (req, res) => {
  res.json({ feature: 'operator-benchmarking', title: 'Operator-benchmarking dashboard comparing peer carriers', project: 'AITelecomNetworkCapacityPlanner' });
});
