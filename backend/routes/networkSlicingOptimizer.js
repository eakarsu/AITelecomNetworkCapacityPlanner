...
// Network slicing optimizer for URLLC/eMBB/mMTC service types
// Feature: network-slicing-optimizer
...
router.post('/analyze', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Network slicing optimizer for URLLC/eMBB/mMTC service types.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'network-slicing-optimizer', payload, result);
    res.json({ success: true, feature: 'network-slicing-optimizer', ...result });
  } catch (err) {
    console.error('network-slicing-optimizer analyze error:', err.message);
...
router.post('/batch', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Network slicing optimizer for URLLC/eMBB/mMTC service types. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'network-slicing-optimizer', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authMiddleware, (req, res) => {
  res.json({ feature: 'network-slicing-optimizer', title: 'Network slicing optimizer for URLLC/eMBB/mMTC service types', project: 'AITelecomNetworkCapacityPlanner' });
});
