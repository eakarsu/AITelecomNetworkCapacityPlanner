...
// 5G deployment planner prioritizing rollout by demand/ROI/competitive pressure
// Feature: 5g-deployment-planner
...
router.post('/analyze', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You are an expert assistant for the "AITelecomNetworkCapacityPlanner" platform. Always return strict JSON only — no markdown, no commentary.',
      `Feature: 5G deployment planner prioritizing rollout by demand/ROI/competitive pressure.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, '5g-deployment-planner', payload, result);
    res.json({ success: true, feature: '5g-deployment-planner', ...result });
  } catch (err) {
    console.error('5g-deployment-planner analyze error:', err.message);
...
router.post('/batch', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You analyze batches of items for "AITelecomNetworkCapacityPlanner". Return strict JSON only.',
      `Feature: 5G deployment planner prioritizing rollout by demand/ROI/competitive pressure. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: '5g-deployment-planner', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authMiddleware, (req, res) => {
  res.json({ feature: '5g-deployment-planner', title: '5G deployment planner prioritizing rollout by demand/ROI/competitive pressure', project: 'AITelecomNetworkCapacityPlanner' });
});
