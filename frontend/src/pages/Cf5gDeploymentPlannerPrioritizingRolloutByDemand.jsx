...
export default function Cf5gDeploymentPlannerPrioritizingRolloutByDemand() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-5g-deployment-planner-prioritizing-rollout-by-demand-roi/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-5g-deployment-planner-prioritizing-rollout-by-demand-roi', project: 'AITelecomNetworkCapacityPlanner' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-5g-deployment-planner-prioritizing-rollout-by-demand-roi/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">5G deployment planner prioritizing rollout by demand/ROI/competitive pressure</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AITelecomNetworkCapacityPlanner</p>
      </div>
