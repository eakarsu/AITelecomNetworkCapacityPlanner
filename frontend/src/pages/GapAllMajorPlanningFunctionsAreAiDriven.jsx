...
export default function GapAllMajorPlanningFunctionsAreAiDriven() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-all-major-planning-functions-are-ai-driven-minimal-gaps/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-all-major-planning-functions-are-ai-driven-minimal-gaps', project: 'AITelecomNetworkCapacityPlanner' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-all-major-planning-functions-are-ai-driven-minimal-gaps/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">All major planning functions are AI-driven; minimal gaps</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_ai · AITelecomNetworkCapacityPlanner</p>
      </div>
