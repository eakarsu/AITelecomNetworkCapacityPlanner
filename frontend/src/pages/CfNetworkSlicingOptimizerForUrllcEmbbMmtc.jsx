...
export default function CfNetworkSlicingOptimizerForUrllcEmbbMmtc() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-network-slicing-optimizer-for-urllc-embb-mmtc-service/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-network-slicing-optimizer-for-urllc-embb-mmtc-service', project: 'AITelecomNetworkCapacityPlanner' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-network-slicing-optimizer-for-urllc-embb-mmtc-service/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Network slicing optimizer for URLLC/eMBB/mMTC service types</h1>
