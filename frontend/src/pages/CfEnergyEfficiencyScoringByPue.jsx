...
export default function CfEnergyEfficiencyScoringByPue() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-energy-efficiency-scoring-by-pue/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-energy-efficiency-scoring-by-pue', project: 'AITelecomNetworkCapacityPlanner' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-energy-efficiency-scoring-by-pue/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Energy efficiency scoring by PUE</h1>
