...
export default function GapNoWhatIfScenarioUiExportTo() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-what-if-scenario-ui-export-to-excel-powerpoint/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-what-if-scenario-ui-export-to-excel-powerpoint', project: 'AITelecomNetworkCapacityPlanner' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-what-if-scenario-ui-export-to-excel-powerpoint/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No what-if scenario UI export to Excel/PowerPoint</h1>
