...
// Feature: Operator-bench-marking dashboard comparing peer carriers
...
export default function CfOperatorBenchMarkingDashboardComparingPeerCarriers() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-operator-bench-marking-dashboard-comparing-peer-carriers/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-operator-bench-marking-dashboard-comparing-peer-carriers', project: 'AITelecomNetworkCapacityPlanner' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-operator-bench-marking-dashboard-comparing-peer-carriers/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Operator-bench-marking dashboard comparing peer carriers</h1>
